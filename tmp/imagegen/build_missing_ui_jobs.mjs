...
const root = "ui-mockups/gpt-image-2/complete-system";
...
for (const style of styles) {
  const outDir = join(root, style.dir);
  mkdirSync(outDir, { recursive: true });
  const lines = [];
  for (const [slug, description] of pages) {
    if (existsSync(join(outDir, `${slug}.png`))) continue;
  ...
  writeFileSync(`tmp/imagegen/zfy-missing-${style.key}.jsonl`, lines.join("\n") + (lines.length ? "\n" : ""), "utf8");
  console.log(`${style.key}: ${lines.length} missing`);
}
