...
<script>
const CACHE_KEY = 'zfy_admin_message_records'
...
export default {
  ...
  created() {
    this.messageList = this.$cache.local.getJSON(CACHE_KEY) || []
  },
  methods: {
    persist() {
      this.$cache.local.setJSON(CACHE_KEY, this.messageList)
    },
    sendMessage() {
    ...
    remove(row) {
      this.$modal.confirm(`确认删除消息 "${row.title}" 吗？`).then(() => {
