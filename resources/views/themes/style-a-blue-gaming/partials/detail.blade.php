...
<section class="a-detail-layout">
    ...
    <article class="a-detail-main">
        ...
        <div class="a-article">
            ...
            <h2>{{ $tone === 'market' ? '一、项目简介' : ($tone === 'creative' ? '一、准备工作' : '一、游戏基础介绍') }}</h2>
            <p>{{ $tone === 'market' ? '本文从技术选型、功能模块、权限设计和落地实践讲解资源内容。' : ($tone === 'creative' ? '开始创作前，需要准备基础工具、素材参考和清晰的创作流程。' : '游戏背景设定在开放世界中，玩家将扮演被选中的旅者，踏上探索世界的冒险之旅。') }}</p>
            <h2>{{ $tone === 'market' ? '二、核心功能实现' : ($tone === 'creative' ? '二、线稿绘制' : '二、全剧情流程攻略') }}</h2>
            <ol>
                <li>完成基础操作教学，解锁主线任务。</li>
                <li>与村长对话接受委托，开启隐藏任务线。</li>
                <li>前往遗迹地图收集关键道具。</li>
            </ol>
        </div>
