...
$(document).ready(function() {
  // 在<input>后面添加自定义按钮
  $('#title').after('<button class="btn primary aibtn" id="chatkey" type="button">ChatGPT文章生成</button>');

  $('#chatkey').on('click', function () {
    var title = $('input[name="title"]').val(); // 获取关键字

    if ($.trim(title) === '') {
      cocoMessage.warning('未配置标题');
    } else {
      $("#chatkey").html('正在生成……'); 
      aititle(title,1);
    }
...
function aititle(keyword,i) {
            ...
            $.ajax({			
				...
				success: function(data) {
				    console.log(data);
				    $(".cm-activeLine.cm-line").append(data.content);
				    cocoMessage.success('生成完毕');
				    if(i==1){
				    $("#chatkey").html('ChatGPT文章生成'); 
				    }
				    
				},
				error: function(err) {
                    console.log(err);
                    cocoMessage.success('生成失败');
                    if(i==1){
				    $("#chatkey").html('ChatGPT文章生成'); 
				    }
